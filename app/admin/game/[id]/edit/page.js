...
export default function EditGame() {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto p-4 space-y-3">
      ...
      <input placeholder="Genre (pisah pakai koma)" value={form.genre || ''} onChange={(e) => setForm({ ...form, genre: e.target.value })} className="w-full bg-white/10 p-3 rounded-xl" />
      <div>
        <label className="text-sm text-gray-400">Status</label>
        <select value={form.status || 'Ongoing'} onChange={(e) => setForm({ ...form, status: e.target.value })} className="w-full bg-white/10 p-3 rounded-xl mt-1">
